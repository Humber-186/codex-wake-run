...
```
   ...
   Stages are ordered, one-shot `log_line_regex` events. One complete log line can advance at most one stage. Use patterns emitted by the actual foreground process; do not infer ambiguous progress rules.
...
- `--detach <run_id>` drains delivery attempts for already-persisted stage events, reports their delivery summary, releases this Run's Goal holder, and ends supervision while leaving the target alive. It deliberately gives up the exact exit code and final notification.
...
- Persist every stage event found by one scan before advancing the runtime checkpoint, then submit delivery. Match complete log lines incrementally and only against the next unfinished stage. Checkpoint matched events and terminal/control boundaries immediately; throttle ordinary offset-only checkpoints to five seconds or 1 MiB. Stage events never release Goal state; replay covers both pending stage and completion events.
...
- Retry terminal delivery failures within `WAKE_RUN_QUEUE_TIMEOUT` (30 minutes by default). Stage delivery uses a shorter `WAKE_RUN_STAGE_QUEUE_TIMEOUT` (20 seconds by default); at target exit, finish at most the active delivery attempt and leave queued events pending for replay. Failures and pending events are summarized in the terminal event. The startup handshake defaults to 10 seconds via `WAKE_RUN_STARTUP_TIMEOUT`, and each Goal RPC defaults to 10 seconds via `WAKE_RUN_GOAL_TIMEOUT`. These values must be finite positive numbers. A prepared worker waits only while its launcher process remains alive; launcher death cancels the uncommitted run and releases any acquired guard.
- An adopt attempt uses a unique handshake identity and a per-Run lock. If startup fails after Goal holder transfer, roll the transfer back; target exit after the committed gate is a normal `observed_exit`, not an adopt startup failure.
- A stop/detach command acknowledgement may return `status: pending` when the durable request was not processed within the foreground wait. Report it as accepted but not yet acknowledged, not as a failed control request.
